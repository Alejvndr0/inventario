            ...
            <div class="form-group">
                ...
                <select name="productos[]" id="productos" class="form-control" multiple required>
                    @foreach ($productos as $producto)
                        @php
                            $selected = is_array($pedido->detalles) ? in_array($producto->id, $pedido->detalles->pluck('producto_id')->toArray()) : false;
                        @endphp
                        <option value="{{ $producto->id }}" @if($selected) selected @endif>{{ $producto->nombre }}</option>
                    @endforeach
                </select>
            </div>
            @foreach ($productos as $producto)
                <div class="form-group">
                    ...
                    <input type="number" id="cantidad_{{ $producto->id }}" name="cantidad[{{ $producto->id }}]" class="form-control" value="{{ is_array($pedido->detalles) ? $pedido->detalles->where('producto_id', $producto->id)->first()->cantidad : 0 }}">
                </div>
            @endforeach
